...
from msal import authority, TokenCache
...
class AuthnClientBase(object):
    ...
    def get_cached_token(self, scopes, auth_url):
        # type: (Iterable[str], str) -> Optional[str]
        _, environment, realm = authority.canonicalize(auth_url)
        query = {"environment": environment, "realm": realm}
    ...
    def _deserialize_and_cache_token(self, response, scopes):
        ...
        try:
            ...
            self._cache.add({"response": payload, "scope": scopes, "token_endpoint": response.http_request.url})
            return token
        except KeyError:
    ...
    def _prepare_request(self, method="POST", auth_url=None, headers=None, form_data=None, params=None):
        # type: (Optional[str], Optional[Mapping[str, str]], Optional[Mapping[str, str]], Optional[Dict[str, str]]) -> HttpRequest
        request = HttpRequest(method, auth_url or self._auth_url, headers=headers)
        if form_data:
...
class AuthnClient(AuthnClientBase):
    ...
    def request_token(self, scopes, method="POST", headers=None, form_data=None, params=None, **kwargs):
        ...
        request = self._prepare_request(method, headers=headers, form_data=form_data, params=params, **kwargs)
