...
from .._internal.persistent_cache import PersistentTokenCache  # importing this from azure.identity creates a cycle
...
class SharedTokenCacheBase(ABC):
    def __init__(self, username=None, **kwargs):  # pylint:disable=unused-argument
        ...
        self._cache = kwargs.pop("token_cache", None)  # type: TokenCache
    ...
    def _load_cache(self):
        if not self._cache and self.supported():
            try:
                ...
                # create a new cache. If the default cache exists, the user must have created it earlier, and
                # perhaps allowed it to be stored without encryption.
