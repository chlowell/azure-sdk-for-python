...
from azure.identity import AuthenticationRequiredError, AuthenticationRecord
...
def test_disable_persistent_cache():
    """The credential should use an in memory cache when given disable_persistent_cache=True"""
    ...
    with patch.multiple("msal_extensions.token_cache", WindowsTokenCache=raise_when_called):
        with patch(InteractiveCredential.__module__ + ".msal.TokenCache", Mock(return_value=expected_cache)):
            credential = MockCredential(client_id="...", disable_persistent_cache=True)
