...
def test_no_scopes():
    ...
    credential = SharedTokenCacheCredential(token_cache=TokenCache())
    with pytest.raises(ValueError):
...
def test_authority(authority):
    ...
    MockCredential(token_cache=TokenCache(), authority=authority, transport=transport)
...
def test_empty_cache():
    ...
    with pytest.raises(CredentialUnavailableError, match=NO_ACCOUNTS):
        SharedTokenCacheCredential(token_cache=TokenCache()).get_token("scope")
    with pytest.raises(CredentialUnavailableError, match=NO_ACCOUNTS):
        SharedTokenCacheCredential(token_cache=TokenCache(), username="not@cache").get_token("scope")
    with pytest.raises(CredentialUnavailableError, match=NO_ACCOUNTS):
        SharedTokenCacheCredential(token_cache=TokenCache(), tenant_id="not-cached").get_token("scope")
    with pytest.raises(CredentialUnavailableError, match=NO_ACCOUNTS):
        SharedTokenCacheCredential(token_cache=TokenCache(), tenant_id="not-cached", username="not@cache").get_token("scope")
...
def test_no_matching_account_for_username():
    ...
    with pytest.raises(CredentialUnavailableError) as ex:
        SharedTokenCacheCredential(token_cache=cache, username="not" + upn).get_token("scope")
...
def test_no_matching_account_for_tenant():
    ...
    with pytest.raises(CredentialUnavailableError) as ex:
        SharedTokenCacheCredential(token_cache=cache, tenant_id="not-" + tenant).get_token("scope")
...
def test_no_matching_account_for_tenant_and_username():
    ...
    with pytest.raises(CredentialUnavailableError) as ex:
        SharedTokenCacheCredential(token_cache=cache, tenant_id="not-" + tenant, username="not" + upn).get_token("scope")
...
def test_authentication_record_empty_cache():
    ...
    credential = SharedTokenCacheCredential(
        authentication_record=record, transport=Mock(send=send), token_cache=TokenCache()
    )
...
def test_authentication_record_no_match():
    ...
    credential = SharedTokenCacheCredential(authentication_record=record, transport=Mock(send=send), token_cache=cache)
...
def test_authentication_record():
    ...
    credential = SharedTokenCacheCredential(authentication_record=record, transport=transport, token_cache=cache)
...
def test_writes_to_cache():
    ...
    credential = SharedTokenCacheCredential(
        token_cache=cache, transport=Mock(send=Mock(side_effect=Exception("the credential should return a cached token")))
    )
    token = credential.get_token(scope)
    assert token.token == expected_access_token
    ...
    assert len(cache._cache.find(msal.TokenCache.CredentialType.REFRESH_TOKEN)) == 1


def test_initialization():
    """the credential should attempt to load the cache only once, when it's first needed"""

    with patch("azure.identity._internal.persistent_cache._get_persistence") as mock_cache_loader:
...
def test_authentication_record_authenticating_tenant():
    ...
    credential = SharedTokenCacheCredential(
        authentication_record=record, token_cache=TokenCache(), tenant_id=expected_tenant_id, transport=transport
    )
    with pytest.raises(CredentialUnavailableError):
...
def populated_cache(*accounts):
    ...
    for account in accounts:
        cache._cache.add(account)
    cache._cache.add = lambda *_, **__: None  # prevent anything being added to the cache
    return cache
